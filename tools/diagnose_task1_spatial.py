...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Trace Task 1 policy behavior on original/spatial maps.")
    parser.add_argument(
        "--map-variant",
...
def main() -> None:
    ...
    if args.map_variant == "default":
        ...
        map_path = materialize_spatial_map_variant(TASK_ID, args.map_variant, seed=args.seed)
        env = make_env(task_id=TASK_ID, map_path=map_path, **env_kwargs)
    ...
    print(
        f"task={TASK_ID} map_variant={args.map_variant} seed={args.seed} "
    ...
    try:
        for step in range(args.max_steps):
            ...
            if should_print:
                print(_info_line(step, action, last_reward, info, agent, symbol_map, step_events))
...
def _info_line(
    ...
    return (
        ...
        f"vis_chests={chests} vis_exits={exits} walls={len(walls)} "
        f"move=({getattr(agent, '_move_action', None)},{getattr(agent, '_move_ticks_remaining', None)}) "
        f"opened={opened} pending=({pending_target},{pending_open}) available={available} "
        f"known_exits={sorted(getattr(agent, '_known_exits', set()))} "
        f"failed={dict(failed)} entities={entities} "
